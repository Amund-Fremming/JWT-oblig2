...
public class OrderService {
	...
	public Order updateOrder(Order order, Long id) throws OrderNotFoundException {
		Order orderToUpdate = orderRepository.findById(id).orElseThrow(() -> new OrderNotFoundException("Order with id: " + id + " not found in the order list"));
		
		orderToUpdate.setExpiry(order.getExpiry());
		orderToUpdate.setIsbn(order.getIsbn());
